...
* Install FFmpeg and make sure `ffmpeg` is available on your PATH. It is needed to process local MP3 clips.
...
* **Play local audio files**: Plays downloaded ten-second clips instead of opening YouTube. Local audio settings can shorten the clip, change playback rate, reverse it, add static from 0 to 100, or combine up to 20 songs into one mix. Combined songs are selected without replacement within each question.
